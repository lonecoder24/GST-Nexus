...
const ClientStatus: React.FC = () => {
  ...
  const clientData = React.useMemo(() => {
      ...
      const mapped = taxpayers.map(t => {
          ...
          clientNotices.forEach(n => {
              const key = n.arn ? `CASE_${n.arn}` : `NOTICE_${n.id}`;
              const label = n.arn ? `Case ID: ${n.arn}` : `Notice: ${n.noticeNumber}`;
              
              if (!caseGroups[key]) {
                  caseGroups[key] = { id: key, label, notices: [], effectiveDate: null, daysSince: Infinity };
              }
              caseGroups[key].notices.push(n);
          });

          let maxDaysSinceForClient = 0;
          let slaBreachCount = 0;

          // Calculate effective status for each group
          const groupsArray = Object.values(caseGroups).map(group => {
              // Find the most recent check date in this group (Checking one notice implies checking the case)
              let latestDateStr: string | null = null;
              
              group.notices.forEach(n => {
                  if (n.lastCheckedDate) {
                      if (!latestDateStr || new Date(n.lastCheckedDate) > new Date(latestDateStr)) {
                          latestDateStr = n.lastCheckedDate;
                      }
                  }
              });

              group.effectiveDate = latestDateStr;
              group.daysSince = getDaysSince(latestDateStr);

              // Client level aggregation
              if (group.daysSince === Infinity) {
                  maxDaysSinceForClient = Infinity;
              } else if (maxDaysSinceForClient !== Infinity && group.daysSince > maxDaysSinceForClient) {
                  maxDaysSinceForClient = group.daysSince;
              }

              if (group.daysSince > SLA_DAYS) {
                  slaBreachCount++;
              }
              
              return group;
          }).sort((a, b) => b.daysSince - a.daysSince); // Sort groups by urgency
  ...
  const handleMarkGroupChecked = async (notices: Notice[]) => {
    ...
    <div className="space-y-6 pb-10">
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            <div className="bg-slate-50 border-b border-slate-200 px-6 py-3 grid grid-cols-12 text-xs font-bold text-slate-500 uppercase">
                ...
                <div className="col-span-3 md:col-span-2 text-center">Active Cases</div>
                                    ...
                                    <div className="bg-slate-50/50 border-y border-slate-100 px-6 py-2">
                                        <div className="ml-8 space-y-3 my-2">
                                            {client.caseGroups.map((group: any) => {
                                                const isCaseBreach = group.daysSince > SLA_DAYS;
                                                
                                                return (
                                                    <div key={group.id} className={`bg-white p-3 rounded-lg border shadow-sm ${isCaseBreach ? 'border-red-200 ring-1 ring-red-50' : 'border-slate-200'}`}>
                                                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2">
                                                            <div className="flex items-start gap-3">
                                                                <div className={`mt-1 p-1.5 rounded-lg ${group.id.startsWith('CASE') ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-500'}`}>
                                                                    {group.id.startsWith('CASE') ? <FolderOpen size={16}/> : <FileText size={16}/>}
                                                                </div>
                                                                <div>
                                                                    <div className="flex items-center gap-2">
                                                                        <span className="text-sm font-bold text-slate-800">{group.label}</span>
                                                                        {group.notices.length > 1 && (
                                                                            <span className="text-[10px] bg-slate-100 text-slate-500 px-1.5 rounded">{group.notices.length} Notices</span>
                                                                        )}
                                                                    </div>
                                                                    <div className="flex flex-wrap gap-1 mt-1">
                                                                        {group.notices.map((n: Notice) => (
                                                                            <span key={n.id} onClick={() => navigate(`/notices/${n.id}`)} className="cursor-pointer text-[10px] bg-white border border-slate-200 text-slate-600 px-1.5 py-0.5 rounded hover:border-blue-300 hover:text-blue-600 transition-colors flex items-center gap-1">
                                                                                {n.noticeNumber} <ExternalLink size={8}/>
                                                                            </span>
                                                                        ))}
                                                                    </div>
                                                                </div>
                                                            ...
                                                            <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 border-slate-100 pt-2 md:pt-0 mt-2 md:mt-0">
                                                                ...
                                                                <button 
                                                                    onClick={() => handleMarkGroupChecked(group.notices)} 
                                                                    className="p-1.5 text-blue-600 hover:bg-blue-50 rounded border border-blue-100 hover:border-blue-300 transition-colors bg-white shadow-sm flex items-center gap-1 text-xs px-2" 
                                                                    title="Mark Case as Checked"
                                                                >
